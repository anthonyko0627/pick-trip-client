...
> **업데이트 (feat/22 완료 후)**: 이 문서 작성 이후 이슈 #22(AI 일정 생성) 구현이
> 먼저 완료되면서, 아래 "1. 타입 정렬" 단계가 이미 반영됐다. `src/types/itinerary.ts`는
> 이 문서가 요구하는 것과 동일하게 `SaveItineraryRequest`/`ItineraryResponse`/`Day`/`Item`
> 로 정렬되어 있고, `generateItinerary()`(요청 바디 없음, 미리보기 반환)/`saveItinerary()`
> (POST, 실제 저장)도 이미 구현/검증됐다. 또한 "오픈 이슈 4번"(편집 시점에 `itineraryId`가
> 실존해야 한다는 우려)도 `ItineraryClient.tsx`의 generate→preview→save 상태 기계로
> 해소됐다: `"saved"` phase(`saveItinerary()` 성공 후, `ItineraryResponse.itineraryId` 확보)
> 에서 편집 기능을 붙이면 된다. 아래 "1. 타입 정렬"과 관련 오픈 이슈 항목은 이 내용을
> 반영해 갱신했다. **2번(`getItinerary`/`modifyItinerary` 추가)부터 실제 구현을 시작한다.**

---

## 현재 상태 요약

- `src/types/itinerary.ts`는 이미 백엔드 실제 계약에 맞춰 `SaveItineraryRequest`/`DayRequest`/`ItemRequest`/`ItineraryResponse`/`Day`/`Item`/`ItineraryGenerateResponse`로 정렬되어 있다 (아래 "1. 타입 정렬" 참고 — 재작업 불필요).
- `src/services/itineraryService.ts`는 `generateItinerary()`(요청 바디 없음, `POST /api/v1/itineraries/generate` 호출, id 없는 미리보기 반환)와 `saveItinerary()`(`POST /api/v1/itineraries`, 실제 저장 후 `itineraryId` 발급)로 분리되어 있다. 이 문서가 추가하려는 `getItinerary`/`modifyItinerary`는 아직 없다.
- `ItineraryClient.tsx`의 `ItineraryPhase` 상태 기계는 `idle | loading | preview | loginPreview | saving | saved | error`로 확장되어 있다. `preview`는 generate 성공 직후(저장 대기, id 없음) 단계, `saved`는 `saveItinerary()` 성공 후(`ItineraryResponse.itineraryId` 확보) 단계다. `loginPreview`는 로그인 기능이 아직 없어 generate가 401 `AUTH_REQUIRED`로 실패할 때 바구니 콘텐츠 기반 로컬 미리보기를 보여주는 임시 상태다. **일정 수정 기능은 `"saved"` phase에 붙인다** — `preview`/`loginPreview` 단계는 아직 `itineraryId`가 없어 PATCH 대상이 없다.
- `ItineraryResult.tsx → DayCard.tsx → PlaceItem.tsx`는 콜백 prop이 하나도 없는 순수 읽기 전용 렌더 체인이며, `dayId`/`itemId`로 키를 잡는다(`Day`/`Item` 타입 정렬 시 함께 갱신됨).
...
## 1. 타입 정렬 — `src/types/itinerary.ts` (✅ 완료됨, feat/22에서 구현)

아래 타입은 이미 `src/types/itinerary.ts`에 그대로 구현되어 있다. 이 섹션은
재작업 대상이 아니라 "일정 수정" 기능이 의존하는 계약을 확인하기 위한 참고용으로
남긴다.
...
`ItineraryGenerateResponse`(id 없는 미리보기 응답)도 이미 `SaveItineraryRequest`/`ItineraryResponse`와 이름이 겹치지 않게 정리되어 있다.

---

## 2. 서비스 레이어 — `src/services/itineraryService.ts` (여기서부터 실제 구현 시작)
...
`generateItinerary()`는 이미 요청 바디 없이 구현/검증되어 있으므로 이번 이슈에서 손대지 않는다. `modifyItinerary`는 전체 교체(full replace) 방식이므로, 편집 UI는 항상 현재 로컬 상태 전체로 `days[]`를 구성해서 보내야 한다.
...
`ItineraryClient.tsx`의 `ItineraryPhase`는 이미 `idle | loading | preview | loginPreview | saving | saved | error`로 구성되어 있다(재정의 불필요). 이 중 `"saved"` 단계만 `ItineraryResponse`(`itineraryId` 포함)를 들고 있으므로, 일정 수정 기능은 `"saved"` 단계 렌더링 분기에서만 `useItineraryEditor({ itineraryId: phase.data.itineraryId, initialDays: phase.data.days })`를 사용하고, 반환된 상태/액션을 `ItineraryResult`로 전달한다. `preview`/`loginPreview` 단계에서는 아직 `itineraryId`가 없으므로 편집 기능을 노출하지 않는다. "다시 생성" 클릭 시 `isDirty`가 true이면 확인 절차를 넣을지는 구현 중 UX 세부사항으로 결정한다 (계획을 막는 사항 아님).
...
- ~~오픈 질문: 편집 화면 도달 시점에 itineraryId가 실존해야 한다~~ → **해소됨**. `ItineraryClient.tsx`의 `"saved"` phase(사용자가 명시적으로 "저장" 버튼을 눌러 `saveItinerary()`가 성공한 뒤)에서만 편집 기능을 노출하므로, 이 단계에 도달했다면 `itineraryId`는 항상 존재한다.
...
2. `/select → /select/conditions → /contents`에서 콘텐츠 2개 이상 담고 일정 생성 → "저장" 클릭까지 진행해 `/itinerary`의 `"saved"` 단계(편집 기능이 붙는 단계) 진입.
...
1. ~~`src/types/itinerary.ts` 타입 정렬~~ — 완료됨 (feat/22), 재작업 불필요
...
2. ~~`POST /api/v1/itineraries/generate` 요청 바디 미문서화~~ → **해소됨**. feat/22 구현/검증 결과 generate는 요청 바디 없이 서버에 저장된 바구니/조건을 읽어 동작한다.
3. `dayIndex`/`order` 0/1-based 여부 — 구현 착수 시 실제 검증.
4. ~~편집 화면 도달 시점에 `itineraryId`가 이미 실존(저장됨)한다는 가정~~ → **해소됨**. 편집 기능을 `"saved"` phase에 붙이므로 항상 `itineraryId`가 존재한다.
